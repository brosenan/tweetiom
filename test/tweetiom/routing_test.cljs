...
;;;;;; render-page ;;;;;;;;;
;; Dispatching is done through the multi-method render-page.
;; render-page takes the content of the page atom, and renders a page.
...
;; For example:
(defmethod route/render-page :foo [[_foo a b c] ctx]
  [:div "This is foo: " a b c ctx])
(defmethod route/render-page :bar [[_bar a b c] ctx]
  [:div "This is bar: " a b c])

;; Now, displaying the current page should dispatch on the keyword.
(deftest render-page-1
  (is (= (route/render-page [:foo 1 2 3] :ctx) [:div "This is foo: " 1 2 3 :ctx]))
  (is (= (route/render-page [:bar 3 2 1] :ctx) [:div "This is bar: " 3 2 1])))

;;;;;; Route ;;;;;;;;;
;; The route function puts this all together.
;; It takes the context as parameter, and calls render-page with the contents of @page and the context.
(deftest route-1
